...
class PostgresStore {
  ...
  getPool() {
    if (!this.connectionString) {
      throw new Error("DATABASE_URL belum diatur. Untuk Vercel, isi Environment Variable DATABASE_URL dengan PostgreSQL online, bukan 127.0.0.1.");
    }
    if (!this.pool) {
  ...
  async ensureConnected() {
    ...
    await this.query("alter table daily_motivation_scores add column if not exists answers jsonb not null default '{}'::jsonb");
  ...
  async getTodayMotivationScore(patientId) {
    const result = await this.query(
      `select score, raw_value, note, answers, created_at
    ...
    return {
      ...
      note: row.note,
      answers: row.answers?.answers || [],
      category: row.answers?.category || null,
      rawTotal: row.answers?.rawTotal || null,
      createdAt: row.created_at
    };
  }
  ...
  async saveMotivationScore(patientId, input) {
    ...
    if (answers.length) {
      if (answers.length !== 5) throw new Error("Survei motivasi harian wajib mengisi 5 pertanyaan singkat");
      const reverseItems = new Set([2]);
      const scored = answers.map((value, index) => reverseItems.has(index) ? 5 - value : value);
      rawTotal = scored.reduce((sum, value) => sum + value, 0);
      score = Math.max(0, Math.min(100, Math.round(((rawTotal - 5) / 15) * 100)));
      rawValue = Math.max(1, Math.min(5, Math.round(score / 20) || 1));
      payloadAnswers = answers;
    } else {
      const legacyValue = Number(input.value || input.rawValue || input.score);
      if (!Number.isFinite(legacyValue) || legacyValue < 1 || legacyValue > 5) {
        throw new Error("Survei motivasi harian wajib diisi");
      }
      rawValue = Math.round(legacyValue);
      score = Math.round((rawValue / 5) * 100);
      rawTotal = rawValue;
    }

    const category = score >= 80 ? "tinggi" : score >= 60 ? "cukup" : score >= 40 ? "rendah" : "sangat rendah";
    const note = String(input.note || "").trim() || null;
    await this.query(
      `insert into daily_motivation_scores (patient_id, score, raw_value, note, answers)
       values ($1, $2, $3, $4, $5::jsonb)`,
      [patientId, score, rawValue, note, JSON.stringify({
        instrument: "Survei Motivasi Harian ASET-TB",
        scale: "SS=4, S=3, TS=2, STS=1; item negatif dibalik",
